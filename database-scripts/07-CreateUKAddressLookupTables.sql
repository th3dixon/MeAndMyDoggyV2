...
-- Create Full-Text Catalog for address searching
IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE name = 'AddressCatalog')
BEGIN
    CREATE FULLTEXT CATALOG AddressCatalog AS DEFAULT;
END

-- Create Full-Text Indexes
IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('dbo.AddressLookupCache'))
BEGIN
    CREATE FULLTEXT INDEX ON [dbo].[AddressLookupCache]
    (
        [SearchKey] LANGUAGE 'English',
        [DisplayText] LANGUAGE 'English',
        [AddressLine1] LANGUAGE 'English',
        [City] LANGUAGE 'English'
    )
    KEY INDEX [PK_AddressLookupCache]
    ON AddressCatalog
    WITH (CHANGE_TRACKING = AUTO, STOPLIST = SYSTEM);
END

IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('dbo.Streets'))
BEGIN
    CREATE FULLTEXT INDEX ON [dbo].[Streets]
    (
        [StreetName] LANGUAGE 'English'
    )
    KEY INDEX [PK_Streets]
    ON AddressCatalog
    WITH (CHANGE_TRACKING = AUTO, STOPLIST = SYSTEM);
END

IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = OBJECT_ID('dbo.Cities'))
BEGIN
...
PRINT 'UK Address Lookup tables created successfully';